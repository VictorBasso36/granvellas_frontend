...
export async function POST(req: NextRequest, res: NextResponse) {
    ...
    try {
        ...
        const mailOption = {
            ...
            subject: `Gran vellas - Contato via site - ${items?.name ||items?.data?.name } da ${items?.data?.company || items?.data?.acting || items?.acting } `,
            html: `
        <h3>Gran vellas - Contato via site</h3>
